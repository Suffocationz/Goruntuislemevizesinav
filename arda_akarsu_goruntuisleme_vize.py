...
def draw_landmarks_on_image(rgb_image, detection_result):
    ...
    for idx in range(len(hand_landmarks_list)):
        ...
        # 🔥 PARLAKLIK AYARI
        parlaklik = int(np.interp(uzaklik, [20, 200], [0, 100]))
        try:
            sbc.set_brightness(parlaklik)
        except Exception as e:
            print("Parlaklık ayarlanamadı:", e)

        # Ekrana parlaklık oranı yaz
        annotated_image = cv2.putText(annotated_image, f"{parlaklik}%", (xort, yort),
...
while cam.isOpened():
    ...
    if basari:
        ...
        cv2.imshow("Parlaklık Kontrol", cv2.cvtColor(annotated_image, cv2.COLOR_RGB2BGR))
